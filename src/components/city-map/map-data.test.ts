...
describe("starter district", () => {
  it("starts with sixty-four empty interactive plots across four blocks", () => {
    ...
    expect(starterDistrict.entities).toHaveLength(299);
    ...
    expect(new Set(starterDistrict.entities.map((entity) => entity.assetId))).toEqual(
      new Set(["map-base", "road-straight", "sidewalk-straight", "grass-plot", "driveway-straight", "roundabout", "palm-tree", "road-link", "launch-monument"]),
    );
  });
